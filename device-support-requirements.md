...
* [Software support](#software-support)
  * [Lineage makefile](#lineagemk)
  * [Lineage repo dependencies](#lineagedependencies)
...
* Devices MUST support CVE and LVT patches for “high profile” exploits and vulnerabilities (if the media is reporting on it, then we must have it patched).
* [NOTE: This will become a MUST once CVE/LVT autopatcher is live & automated]
* Devices SHOULD receive regular CVE and LVT patches to the device kernel and dependencies.
* [To be in effect once CVE/LVT autopatcher is live & automated]
* Device maintainers MUST review and/or accept patches provided by the CVE/LVT autopatcher tool.
...
__This applies only to devices that have a stock ROM providing a security patch level__

* All devices MUST report a valid vendor security patch level.
* This value MUST match the value of the security patch level of the stock ROM from which blobs are pulled.
* Devices that do not a have a vendor image that reports a valid `ro.vendor.build.security_patch` value MUST supply the security patch level in the `ro.lineage.build.vendor_security_patch` property.
